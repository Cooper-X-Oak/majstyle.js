...
export function getPlayerDatas() {
  ...
  try {
    if (
      gameWindow &&
      gameWindow.view &&
      gameWindow.view.DesktopMgr &&
      gameWindow.view.DesktopMgr.Inst &&
...
// 判断是否在整桌游戏中（不是小局）
// 修复：使用 ingame 判断整桌游戏状态，而不是 gameing（gameing 是小局状态）
// ingame=true 代表整桌游戏会话建立，即使小局结束也保持 true，直到退出牌桌
  ...
  try {
    if (
      !(
        gameWindow &&
        gameWindow.GameMgr &&
        gameWindow.GameMgr.Inst &&
        gameWindow.GameMgr.Inst.ingame
      )
    ) {
      return false;
    }
    // 只检查 player_datas 是否存在，不检查 gameing
    var dm =
      gameWindow.view &&
      gameWindow.view.DesktopMgr &&
      gameWindow.view.DesktopMgr.Inst;
    return !!(dm && dm.player_datas && dm.player_datas.length > 0);
  } catch (e) {
    return false;
  }
}
    ...
    if (hasStatsKeyword) {
      ...
      if (Array.isArray(value)) {
        ...
        if (value.length > 0) {
          console.log(
            "   第一个元素:",
            JSON.stringify(value[0]).substring(0, 200),
          );
        }
...
export function exploreGameObject() {
  ...
  var statsRelatedKeys = topLevelKeys.filter(function (key) {
    return (
      key.toLowerCase().indexOf("stat") !== -1 ||
