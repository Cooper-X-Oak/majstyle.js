...
// @description  金之间/玉之间四人麻将对手风格实时分析（基于牌谱屋数据）- Phase 2.1 配置化系统
...
(function () {
  ...
  function evaluateAttackStrength(stats) {
    ...
    // 1. 立直进攻 (30% 权重)
    var riichiScore = calculateRiichiAttack(stats);
    scores.立直进攻 = riichiScore;
    weightedSum += riichiScore * 0.30;
    totalWeight += 0.30;

    // 2. 副露进攻 (25% 权重)
    var fuluScore = calculateFuluAttack(stats);
    scores.副露进攻 = fuluScore;
    weightedSum += fuluScore * 0.25;
    totalWeight += 0.25;

    // 3. 速度 (25% 权重)
    var speedScore = calculateSpeed(stats);
    scores.速度 = speedScore;
    weightedSum += speedScore * 0.25;
    totalWeight += 0.25;

    // 4. 打点 (20% 权重)
    var datenScore = calculateDaten(stats);
    scores.打点 = datenScore;
    weightedSum += datenScore * 0.20;
    totalWeight += 0.20;
    ...
    // 生成态度词
    var attitude = generateAttackAttitude(stats, scores);
  ...
  function evaluateDefenseStrength(stats) {
    ...
    // 1. 放铳控制 (40% 权重)
    var dealScore = calculateDealControl(stats);
    scores.放铳控制 = dealScore;
    weightedSum += dealScore * 0.40;
    totalWeight += 0.40;

    // 2. 大牌防守 (30% 权重)
    var bombScore = calculateBombDefense(stats);
    scores.大牌防守 = bombScore;
    weightedSum += bombScore * 0.30;
    totalWeight += 0.30;

    // 3. 隐蔽性 (20% 权重)
    var concealScore = calculateConcealment(stats);
    scores.隐蔽性 = concealScore;
    weightedSum += concealScore * 0.20;
    totalWeight += 0.20;

    // 4. 立直防守 (10% 权重)
    var riichiDefenseScore = calculateRiichiDefense(stats);
    scores.立直防守 = riichiDefenseScore;
    weightedSum += riichiDefenseScore * 0.10;
    totalWeight += 0.10;
    ...
    // 生成态度词
    var attitude = generateDefenseAttitude(stats, scores);
  ...
  // 立直进攻得分
  function calculateRiichiAttack(stats) {
    ...
    // 立直率得分 (0-10)
    var rateScore = normalizeScore$1(riichiRate, 10, 30, 0, 10);

    // 立直收支得分 (0-10)
    var profitScore = normalizeScore$1(riichiProfit, -1e3, 3000, 0, 10);

    // 立直后和牌率得分 (0-10)
    var winScore = normalizeScore$1(riichiWinRate, 30, 55, 0, 10);

    // 综合得分
    return rateScore * 0.4 + profitScore * 0.4 + winScore * 0.2;
  }

  // 副露进攻得分
  function calculateFuluAttack(stats) {

    var fuluRate = (stats['副露率'] || 0) * 100;
    var fuluWinRate = (stats['副露后和牌率'] || 0) * 100;

    // 副露率得分 (0-10)
    var rateScore = normalizeScore$1(fuluRate, 20, 45, 0, 10);

    // 副露后和牌率得分 (0-10)
    var winScore = normalizeScore$1(fuluWinRate, 20, 40, 0, 10);

    // 综合得分
    return rateScore * 0.5 + winScore * 0.5;
  }

  // 速度得分
  function calculateSpeed(stats) {

    var avgTurn = stats['和了巡数'] || 12;

    // 和了巡数得分 (0-10)，巡数越小得分越高
    return normalizeScore$1(avgTurn, 14, 9, 0, 10);
  }

  // 打点得分
  function calculateDaten(stats) {

    var avgDaten = stats['平均打点'] || 6500;
    var avgZimo = (stats['自摸率'] || 0) * 100;

    // 平均打点得分 (0-10)
    var datenScore = normalizeScore$1(avgDaten, 5500, 7500, 0, 10);

    // 自摸率得分 (0-10)
    var zimoScore = normalizeScore$1(avgZimo, 5, 15, 0, 10);

    // 综合得分
    return datenScore * 0.7 + zimoScore * 0.3;
  }

  // ========== 防守子维度计算 ==========

  // 放铳控制得分
  function calculateDealControl(stats) {

    var dealRate = (stats['放铳率'] || 0) * 100;
    var avgDealPoint = stats['平均铳点'] || 5000;

    // 放铳率得分 (0-10)，放铳率越低得分越高
    var rateScore = normalizeScore$1(dealRate, 18, 11, 0, 10);

    // 平均铳点得分 (0-10)，铳点越低得分越高
    var pointScore = normalizeScore$1(avgDealPoint, 5500, 4500, 0, 10);

    // 综合得分
    return rateScore * 0.7 + pointScore * 0.3;
  }

  // 大牌防守得分
  function calculateBombDefense(stats) {

    var bombRate = (stats['被炸率'] || 0) * 100;

    // 被炸率得分 (0-10)，被炸率越低得分越高
    return normalizeScore$1(bombRate, 12, 5, 0, 10);
  }

  // 隐蔽性得分
  function calculateConcealment(stats) {

    var motenRate = (stats['默听率'] || 0) * 100;

    // 默听率得分 (0-10)
    return normalizeScore$1(motenRate, 5, 20, 0, 10);
  }

  // 立直防守得分
  function calculateRiichiDefense(stats) {

    var riichiDealRate = (stats['立直后放铳率'] || 0) * 100;

    // 立直后放铳率得分 (0-10)，放铳率越低得分越高
    return normalizeScore$1(riichiDealRate, 15, 5, 0, 10);
  }

  // ========== 态度词生成 ==========

  // 生成进攻态度词
  function generateAttackAttitude(stats, scores) {
    ...
    // 立直倾向
    if (riichiRate > 25 && scores.立直进攻 > 7) {
      attitudes.push('立直狂');
    } else if (riichiRate > 20 && scores.立直进攻 > 5) {
      attitudes.push('立直型');
    }

    // 副露倾向
    if (fuluRate > 38 && scores.副露进攻 > 7) {
      attitudes.push('副露狂');
    } else if (fuluRate > 35 && scores.副露进攻 > 5) {
      attitudes.push('副露型');
    }

    // 速度倾向
    if (avgTurn < 11 && scores.速度 > 7) {
      attitudes.push('速攻狂');
    } else if (avgTurn < 12 && scores.速度 > 5) {
      attitudes.push('速攻型');
    }
  ...
  // 生成防守态度词
  function generateDefenseAttitude(stats, scores) {
    ...
    // 防守强度
    if (dealRate < 13 && scores.放铳控制 > 7) {
      return '铁壁';
    } else if (dealRate < 15 && scores.放铳控制 > 5) {
      return '防守稳健';
    } else if (dealRate > 17) {
      return '防守弱';
    }
    ...
    // 隐蔽性
    if (motenRate > 15 && scores.隐蔽性 > 7) {
      return '隐蔽高手';
    }

    // 大牌防守
    if (bombRate > 10 && scores.大牌防守 < 4) {
      return '大牌防守差';
    }
    return '防守正常';
  }
  ...
  function printAnalysis(playerData, analysis, baseline, index, isSelf, stats) {
    ...
    // 传递强度信息到 UI
    createPlayerInfoUI(index, 主称号, 标签, 数据, 偏差, baseline, playerData.nickname, isSelf, advice.原型, advice.进攻强度);
