...
(function () {
  ...
  function isInGame() {
    var gameWindow = getGameWindow();
    try {
      if (!(gameWindow && gameWindow.GameMgr && gameWindow.GameMgr.Inst && gameWindow.GameMgr.Inst.ingame)) {
        return false;
      }
  ...
  // 背景与阴影
  var BACKGROUNDS = {
    card: 'rgba(12,10,28,0.92)',
    // 深暗底，偏紫调增加层次感
    cardDim: 'rgba(8,8,20,0.55)',
    // 无数据状态
  ...
  var LAYOUT = {
    ...
    positions: {
      ...
      opponents: ['top: 140px; right: 10px;', 'top: 10px; left: 50%; transform: translateX(-50%);', 'top: 140px; left: 10px;']
    }
  };
  ...
  // 构建渐变边框容器样式（默认展开尺寸，含毛玻璃）
  function buildCardStyle(rarity) {
    var bg = BACKGROUNDS.card;
    return ['position: fixed', 'border: 1px solid transparent', 'background: linear-gradient(' + bg + ', ' + bg + ') padding-box, linear-gradient(135deg, ' + rarity.from + ', ' + rarity.to + ') border-box', 'box-shadow: 0 0 10px ' + rarity.glow + ', ' + BACKGROUNDS.shadow, 'backdrop-filter: blur(6px)', '-webkit-backdrop-filter: blur(6px)', 'color: ' + COLORS.text, 'padding: ' + LAYOUT.padding, 'border-radius: ' + LAYOUT.borderRadius, 'font-size: ' + TYPOGRAPHY.data.size, 'z-index: ' + LAYOUT.zIndex, 'pointer-events: auto', 'width: auto', 'max-width: ' + LAYOUT.maxWidthExpanded, 'cursor: pointer', 'transition: opacity 0.2s ease, max-width 0.2s ease, box-shadow 0.2s ease'].join('; ') + '; ';
  }
  ...
  function createPlayerInfoUI(index, 主称号, 标签, 数据, 偏差, baseline, nickname, isSelf, archetype, advice) {
    ...
    // ── 标题行（始终可见，点击折叠/展开）──
    var titleHtml = '<div class="card-title" style="font-size: ' + TYPOGRAPHY.title.size + '; font-weight: ' + TYPOGRAPHY.title.weight + '; white-space: nowrap;">';
    ...
    titleHtml += '<span class="player-name" style="color: ' + COLORS.playerName + '; font-size: ' + TYPOGRAPHY.tag.size + '; font-weight: normal; margin-left: 4px;">' + escapeHtml(nickname) + (isSelf ? ' [你]' : '') + '</span>';
    titleHtml += '</div>';

    // ── 标签行 ──
    var tagHtml = '';
    if (标签.length > 0) {
      tagHtml = '<div style="color: ' + COLORS.tag + '; font-size: ' + TYPOGRAPHY.tag.size + '; letter-spacing: 0.3px; margin-bottom: 4px;">' + 标签.slice(0, 4).map(escapeHtml).join(' · ') + '</div>';
    }
    ...
    // ── 卡片主体（可折叠，带滑动动画）──
    var bodyHtml = '<div class="card-body" style="overflow: hidden; max-height: 400px; opacity: 1; margin-top: 3px; transition: max-height 0.25s ease, opacity 0.18s ease;">' + tagHtml + divider + dataHtml + '</div>';
    container.innerHTML = titleHtml + bodyHtml;
    document.body.appendChild(container);

    // 点击：展开 ↔ 折叠（折叠时背景透明+边框提亮，仅显示标题行）
    var collapsed = false;
    var expandedBg = 'linear-gradient(' + BACKGROUNDS.card + ', ' + BACKGROUNDS.card + ') padding-box, linear-gradient(135deg, ' + rarity.from + ', ' + rarity.to + ') border-box';
    var collapsedBg = 'linear-gradient(rgba(12,10,28,0.35), rgba(12,10,28,0.35)) padding-box, linear-gradient(135deg, ' + rarity.from + ', ' + rarity.to + ') border-box';
    container.addEventListener('click', function (e) {
      e.stopPropagation();
      collapsed = !collapsed;
      var body = container.querySelector('.card-body');
      var nameSpan = container.querySelector('.card-title .player-name');
      if (collapsed) {
        body.style.maxHeight = '0';
        body.style.opacity = '0';
        container.style.background = collapsedBg;
        container.style.boxShadow = '0 0 18px ' + rarity.glow.replace('0.4', '0.9') + ', 0 0 6px ' + rarity.from + ', ' + BACKGROUNDS.shadow;
        if (nameSpan) nameSpan.style.color = 'rgba(255,255,255,0.85)';
        container.style.maxWidth = LAYOUT.maxWidthCompact;
      } else {
        body.style.maxHeight = '400px';
        body.style.opacity = '1';
        container.style.background = expandedBg;
        container.style.boxShadow = '0 0 10px ' + rarity.glow + ', ' + BACKGROUNDS.shadow;
        if (nameSpan) nameSpan.style.color = COLORS.playerName;
  ...
  function processPlayer(p, myId, index) {
    ...
    return Promise.all([getPlayerStats(p.account_id), getPlayerExtendedStats(p.account_id)]).then(function (results) {
      ...
      // 从 player_stats 提取顺位数据，合并到 extStats
      var 平均顺位 = typeof basicStats.avg_rank === 'number' ? basicStats.avg_rank : null;
      var 平均得点 = null;
      if (Array.isArray(basicStats.rank_rates) && Array.isArray(basicStats.rank_avg_score) && basicStats.rank_rates.length === 4 && basicStats.rank_avg_score.length === 4) {
        平均得点 = basicStats.rank_rates[0] * basicStats.rank_avg_score[0] + basicStats.rank_rates[1] * basicStats.rank_avg_score[1] + basicStats.rank_rates[2] * basicStats.rank_avg_score[2] + basicStats.rank_rates[3] * basicStats.rank_avg_score[3];
      }
      if (平均顺位 !== null) {
        extStats['平均顺位'] = 平均顺位;
      }
      if (平均得点 !== null) {
        extStats['平均得点'] = Math.round(平均得点);
      }
      ...
      console.log('    平均顺位:', extStats['平均顺位']);
      console.log('    平均得点:', extStats['平均得点']);
